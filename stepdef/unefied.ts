...
import { commonXpaths } from "../pages/commonPage";
import { Base } from "../pages/BaseFunctions";
import { unefied } from "../pages/unefiedpage";
...
var commonpage = new commonXpaths();
// var casenote = new Casenote;
var unefiedpage = new unefied();
// var commonpage = new EngagementRequest();
const obj = new Base();
...
Then('user gives email as {string}', async function (string) {
    await obj.replaceXpath(commonpage.inputtype, "email").sendKeys(string);
    const screenShot = await browser.takeScreenshot();
    this.attach(screenShot, "image/png");
});

Then('user gives password as {string}', async function (string) {
    await obj.replaceXpath(commonpage.inputtype, "password").sendKeys(string);
    const screenShot = await browser.takeScreenshot();
    this.attach(screenShot, "image/png");
});
Then('user clicks on submit button', async function () {
    await unefiedpage.submituicbtn.click();
    const screenShot = await browser.takeScreenshot();
    this.attach(screenShot, "image/png");
});
...
Then('Enter data in Add Member Page with all details', async function (dataTable) {
    ...
    await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "firstName"), data1.fname);
    await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "middleName"), data1.mname);
    // lastname = await databasewithparm("CMR");
    // console.log(lastname);
    // nmae.name1() 
    await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "lastName"), "Member");
    await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "birthDate"), data1.dob);
    await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "preferredName"), data1.pname);
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Gender (Assigned at Birth) *"),
        obj.replaceXpath(commonpage.matoption, data1.gender));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Gender Identity"),
        obj.replaceXpath(commonpage.matoption, data1.gender));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Pronoun"),
        obj.replaceXpath(commonpage.matoption, data1.PRONOUN));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Sexual Orientation"),
        obj.replaceXpath(commonpage.matoption, data1.SEXUAL_ORIENTATION));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Marital Status"),
        obj.replaceXpath(commonpage.matoption, data1.marital_status));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Primary Language"),
        obj.replaceXpath(commonpage.matoption, data1.PRIMARY_LANGUAGE));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Time Zone"),
        obj.replaceXpath(commonpage.matoption, data1.TIME_ZONE));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Ethnicity"),
        obj.replaceXpath(commonpage.matoption, data1.ETHNICITY));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Race"),
        obj.replaceXpath(commonpage.matoption, data1.RACE));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Lead Type"),
        obj.replaceXpath(commonpage.matoption, data1.LEAD_TYPE));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Lead Source"),
        obj.replaceXpath(commonpage.matoption, data1.LEAD_SOURCE));
    // await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "email"), obj2.Mail());
    // mobilenumber = await phnumbeer("21255663");
    // console.log(mobilenumber);
    // await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "mobile_number"), mobilenumber);
    await obj.setInputField(obj.replaceXpath(commonpage.inputformcontrol, "address"), data1.address);
    await obj.WaitforElementClickable(obj.replaceXpath(commonpage.spantext, "Click here to enter the address manually"));
    await obj.clickElement(obj.replaceXpath(commonpage.spantext, "Click here to enter the address manually"));
    ...
    await browser.actions().mouseMove(await obj.replaceXpath(commonpage.spantext, "Additional Information")).perform();
    await obj.clickElement(obj.replaceXpath(commonpage.spantext, "Additional Information"));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Housing Status"),
        obj.replaceXpath(commonpage.matoption, data1.Housing_status));
    await obj.setDropdownField(obj.replaceXpath(commonpage.mmrselect, "Religious Affiliation"),
        obj.replaceXpath(commonpage.matoption, data1.Religious_Affiliation));
    await obj.setInputField(await obj.replaceXpath(commonpage.textAreaFormcontrol, "physicalDescription"), data1.Note);
...
Then('Enter data in Add Program page', async function (dataTable) {
    ...
    const data = data1[0];
    await obj.clickElement(unefiedpage.none);
    await obj.WaitforElementClickable(await obj.replaceXpath(unefiedpage.plus, data.PARENT_PROGRAM));
    await obj.clickElement(await obj.replaceXpath(unefiedpage.plus, data.PARENT_PROGRAM));
    await obj.clickElement(await obj.replaceXpath(unefiedpage.spancontains, data.program));
    await obj.setInputField(await obj.replaceXpath(unefiedpage.inputdataplaceholder, "EMR ID"), data.EMR_ID);
    await obj.setInputField(await obj.replaceXpath(unefiedpage.inputdataplaceholder, "Medicaid ID"), data.medicaid);
    await obj.setInputField(await obj.replaceXpath(unefiedpage.inputdataplaceholder, "Alternate Identifier"), data.alt_identifier);
    await obj.setInputField(await obj.replaceXpath(unefiedpage.inputdataplaceholder, "Insurance Note"), data.insurance_note);
    await obj.clickElement(await obj.replaceXpath(commonpage.mmrselect, "Provider *"));
    await obj.clickElement(await obj.replaceXpath(unefiedpage.spancontains, data.provider));
    // await obj.clickElement(await obj.replaceXpath(commonpage.mmrselect, "Member Type"));
    // await obj.clickElement(await obj.replaceXpath(commonpage.spancontains, data.Member_Type));
    const screenShot = await browser.takeScreenshot();
    this.attach(screenShot, "image/png");
...
Then('click on Save button', async function () {
    ...
    await obj.clickElement(unefiedpage.popupsave);
...
Then('Verify Data in Care Team Page', async function (dataTable) {
    const data = dataTable.hashes();
    const data1 = data[0];
    let presence: any = await obj.Presence(obj.replaceXpath(commonpage.divtext, data1.ROLE));
    if (presence === "YES") {
        obj.CompareValues("Role", await obj.getTEXT(await obj.replaceXpath(commonpage.divtext, data1.ROLE)), data1.ROLE);
    } else {
        obj.CompareValues("Role", "Not Present", data1.ROLE);
    }
    const value1: any = await obj.getTEXT(await obj.replaceXpath(commonpage.divtext, data1.MEMBER_NAME));
    obj.CompareValues("membername", value1, data1.MEMBER_NAME);
    let yes: any = obj.replaceXpath(unefiedpage.primary, data1.ROLE).isPresent();
    if (yes) {
        yes = 'Yes';
    } else {
        yes = 'No';
...
Then('user clicks on view More', async function (dataTable) {
    var data = dataTable.rowsHash();
    await browser.actions().mouseMove(await obj.replaceXpath(unefiedpage.header, data.section)).perform();
...
Then('verify mandatory fields text', async function (dataTable) {
    ...
    for (let i = 0; i < data.length; i++) {
        const text: any = await obj.getTEXT(await obj.replaceXpath(unefiedpage.divcontains, data[i][0]));
...
Then('verify sub text in demographics page', async function (dataTable) {
    ...
    for (let i = 0; i < data.length; i += 1) {
        let text: any = await obj.getTEXT(await obj.replaceXpath(unefiedpage.sectiontitle, data[i][1]));
        console.log(text);
